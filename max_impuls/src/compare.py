...
#         print(np.array(image).shape)
        ...
        L_R=metric.Residual_loss(image,fake)
        L_D=metric.Discriminator_loss(image, fake,discriminator)*k_rd
        loss=L_R+L_D
        m=np.append(m,loss)
...
def main(disc_path,gen_path,noise_dim,save):
    ...
    train = get_data(trainable)[0] # test dataset from model which is used in training
    test = get_data(not_trainable)[0] # test dataset from model which is not used in training
    df=pd.DataFrame(columns=['name','metric'])
    print("____start____")
    m_full_train,ampl_full_train=metric_main(generator,discriminator,train,noise_dim=noise_dim,batch=128)
    m_full_test,ampl_full_test=metric_main(generator,discriminator,train,noise_dim=noise_dim,batch=128)
    train_name='_'.joint(trainable)
    test_name='_'.joint(not_trainable)
    ...
    df[f'train__{test_name}__mertic'] = m_full_test
    df[f'train__{test_name}__ampl'] = ampl_full_test
    df.to_csv(save)
