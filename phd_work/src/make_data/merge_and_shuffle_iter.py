...
h5s = ['photon_14yr_0001_excl_sat_T_excl_geo_T.h5', 'pr_q4_14yr_e1_0001_excl_sat_T_excl_geo_T.h5']
...
with h5.File(h5_out,'w') as ho:
    ...
    for key in tqdm(keys_to_pull[:]):
        proc = 0
        proc_hits = 0
        with tqdm(total=total_num, desc=f"Processing {key}", leave=False) as pbar:
