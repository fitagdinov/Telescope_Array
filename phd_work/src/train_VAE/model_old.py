...
class Encoder_Transformer(nn.Module):
    ...
    def forward(self,x):
        ...
        return mu, log_var, (None, None)
    def load(self, path):
        if path is not None:
            self.load_state_dict(torch.load(path))
class DecoderRNN_OLD(nn.Module):
    """
    ...
    def __init__(self, latent_dim, hidden_size, output_size, start_token: Tensor, lstm2: bool = False, lstm3: bool = False, num_part: int = 2, **kwargs):
        super(DecoderRNN_OLD, self).__init__()
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_hidden, seq_len):
        ...
        if self.reconstruction_params is not None:
            ...
            recon_pred = None
        return decoder_outputs, (None,None), num, torch.concat(recon_pred, dim=1)
    def seq_LSTMs(self, x, hidden = None):
        """
        Последовательное применение до трёх LSTM-слоёв в декодере.
...
class VAE(nn.Module):
    ...
    def __init__(self, input_dim=6, hidden_dim=32, latent_dim=16, start_token: Tensor = torch.zeros(1,6), lstm2: bool = False, lstm3: bool = False, num_part: int = 2,
        ...
        # self.decoder = DecoderRNN_OLD(latent_dim, hidden_dim_decoder, input_dim, start_token,
    ...
    def forward(self, x):
        ...
        recon_x, _, num, mass = self.decoder(z, seq_len, x[:,:,:3])
...
if __name__ == "__main__":
    model = VAE(stop_token=-11, padding_value=-10)
