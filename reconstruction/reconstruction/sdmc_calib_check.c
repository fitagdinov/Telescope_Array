...
int counter_read_out[NCOUNTERS];
...

{
  ...
  integer4 rcode, arg, ssf, i, j, M, Y, ta_period, k, flg;
  ...
  if (argc == 1) 
    {
      ...
      fprintf(stderr,"a format that can be readily used by the TA SD Monte-Carlo program and then ");
      fprintf(stderr,"just check if the counter is alive or not using the same criteria as TA SD MC\n");
      fprintf(stderr,"Counters that are not working are printed into an ASCII file\n");
      ...
      fprintf(stderr,"     -o <string> : output ASCII file name for dead counters\n\n");
      exit(1);
    }
      ...
      for (;;) 
	{
	  ...
	  for (j = 0; j < tasdcalib_.num_det; j++ )
	      ...
	      if (k < NCOUNTERS)
		{
		  ...
		  // Now, check if the counter is working or dead
		  if((flg=failed_Ben_Stokes_D_Ivanov_Live_Counter_Criteria(buf)))
		    {
		      // if the counter is dead then print the information
		      int yymmdd=0,hhmmss=0;
		      TADay2Date(buf[0]/144.0,&yymmdd,&hhmmss);
		      fprintf(fout,"%06d %06d %04d %d %d\n",
			      tasdcalib_.date,tasdcalib_.time,tasdcalib_.sub[j].lid,flg,
			      (int)(tasdcalib_.sub[j].dontUse & 0xFF));
		    }
		}
	    }
	  // Now print out counters that are not there at all
	  for (k=0; k < NCOUNTERS; k++)
	    {
	      if(counter_read_out[k] < 1)
		fprintf(fout,"%06d %06d %04d %d %d\n",
			tasdcalib_.date,tasdcalib_.time,sdcoor0[k][0],0,0);
	    }
	}
