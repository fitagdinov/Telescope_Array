...
sdmc_calib_check=$TASDMC_BINDIR/sdmc_calib_check.run
test ! -x $sdmc_calib_check && echo "error: executable $sdmc_calib_check not found" >&2 && exit 2

if [ $# -ne 3 ]; then
    echo "Runs calibration checkion program sdmc_calib_check.run on ICRR calibration directory" >&2
    echo "sdmc_calib_check.run reads ICRR calibration DST files and produces sdcalib.bin files" >&2
    echo "that can be readily used by the sdmc_spctr propgram."
    echo "(1): Input ICRR Calib-2 directory" >&2
    echo "(2): output directory for sdcalib_check.txt files" >&2
...
function process_list_files_range() {
	...
	ifl=$tmpdir/in_${i}
	ofl=$outdir/sdcalib_check_${i}.txt
	log=$outdir/out_${i}.log
	cat $ifl | xargs $sdmc_calib_check -c $tasdconst_pass2 -o $ofl >& $log
    done
}
