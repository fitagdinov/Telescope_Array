...
static void fcn_plane(integer4 &npar, real8 *gin, real8 &f, real8 *par,
    integer4 iflag)
  {
    ...
    chisq += ((cogXY[0]-par[2])*(cogXY[0]-par[2])+(cogXY[1]-par[3])*(cogXY[1]
        -par[3])) / (P2GEOM_TEMP_dr*P2GEOM_TEMP_dr);
    f = chisq;
  }
...
p2geomfitter_class::p2geomfitter_class()
  {
    gMinuit = 0;
...
bool p2geomfitter_class::loadVariables_rufptn()
  {
    integer4 i;
    real8 xy[2];
    ...
    for (i=0; i < rufptn_.nhits; i++)
      {
        // Load variables in space-time cluster
...
integer4 p2geomfitter_class::doFit(integer4 whatFCN)
  {
    ...
    gMinuit->SetMaxIterations(P2GEOM_MITR);
...
integer4 p2geomfitter_class::cleanClust(real8 deltaChi2)
  {
...
void p2geomfitter_class::save_fpars()
  {
    theta_old = theta;
    phi_old = phi;
...
void p2geomfitter_class::restore_fpars()
  {
    theta = theta_old;
    phi = phi_old;
...
bool p2geomfitter_class::hasConverged()
  {
