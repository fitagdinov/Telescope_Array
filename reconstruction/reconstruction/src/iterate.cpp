...
static sdxyzclf_class iter_sdxyzclf;

#define enscaleFD 0.78740
...
double g_NL1, g_NL2, g_NL3, g_NZ, g_NT, g_Ndof; // chi2 split
...
double phi_ivanov(double phi) { return ((phi+180.0)-floor((phi+180.0)/360.0)*360.0); };
double mindist(int site, double xcore, double ycore);
double RMS_S(double S, double area);
double E_gamma(double S800, double theta);
...
void iter(sddstio_class& sdrun){
  ...
  if(cmd_opt.isset("rlost") && rusdraw_.event_code == 0) {
    ...
    printf("%s %i %i %i"
	   ...
	   (tasdcalibev_.sim.primaryEnergy>1e-9)?tasdcalibev_.sim.primaryAzimuth:phi_ivanov(rusdmc_.phi*180.0/M_PI),
  ...
  if((!sdrun.haveBank(RUSDRAW_BANKID) || !sdrun.haveBank(RUSDGEOM_BANKID)) && !cmd_opt.isset("junk")) {
    return;
  }

  // If BSDINFO bank is absent then assume there are no mis-calibrated or bad SDs
  // this will not affect the surroundedness cut
  if(!sdrun.haveBank(BSDINFO_BANKID))
  {
	  bsdinfo_.nbsds = 0;
	  bsdinfo_.nsdsout = 0;
  }
  ...
  double P;
  double ndof0, ndof1, ndof_gri, chi2_gri;
  ...
  if(cmd_opt.isset("sdii")) {
     printf("SDII %s%i %i %i %i %i %i %i %i %i %g %g %g %g %g %g %g %g %g %g %g %g %g %i %i %i %i %g %g %g",
	   ((y<10)?"0":""),
	    ...
	    phi_ivanov(rusdgeom_.phi[anal]), rusdgeom_.dphi[anal],
	    rusdgeom_.theta[anal], rusdgeom_.dtheta[anal],
	    (P>-0.1)?mytheta:-1.0, (P>-0.1)?phi_ivanov(myphi):-1.0,
  ...
  double db_phi_gri = (P>-0.1)?phi_ivanov(myphi):-1.0;
...
if(!cmd_opt.isset("ML")) {
  ...
  if(cmd_opt.isset("ML")) {
//Cuts for anisotropy studies (E > 10 EeV). For correctness of 'Ivanov cuts' flag cut energy is rescaled to FD scale (1/1.27)
    int cuts_anisotropy = 1;
    int cuts_spectrum = 1;
    int cuts_gamma = 1;
    int cuts_composition = 1;

    double gfchi2pdof;
    double ldfchi2pdof;
    double energy = rusdenergy(rufldf_.s800[0],rusdgeom_.theta[2]) * enscaleFD; //energy is rescaled to FD scale
    if (rusdgeom_.ndof[2] > 0)
      {
	gfchi2pdof = rusdgeom_.chi2[2] / (double) rusdgeom_.ndof[2];
      }
    else  {  gfchi2pdof = rusdgeom_.chi2[2];  }
    if (rufldf_.ndof[0] > 0)
    ...
    // LDF Fit chi2 / dof cut
    if (ldfchi2pdof > 4.0){
      cuts_anisotropy = 0;
      cuts_spectrum = 0;
      cuts_composition = 0;
    }
    // Joint LDF and geometry fit (Grisha) chi2 / dof cut
    if (P > 5.0){
      cuts_gamma = 0;
      cuts_composition = 0;
    }
    // Pointing direction error cut ( should be less than 5 degrees ) 
    if (pdErr(rusdgeom_.theta[2], rusdgeom_.dtheta[2], rusdgeom_.dphi[2]) > 5.0){
      cuts_anisotropy = 0;
      cuts_spectrum = 0;
      cuts_composition = 0;
    }
    ...
    // 'Anisotropy cuts' for border distance
    integer4 event_surroundedness = iter_sdxyzclf.get_event_surroundedness(&rusdraw_,&rusdgeom_,&bsdinfo_);
    if ( event_surroundedness < 1 )
      cuts_anisotropy = 0;
    // 'Spectrum cuts' for border distance, deprecated in this version
    // Distance from the surrounding edge should be less than 1200m
    ...
    if ( E_gamma(db_S800_gri, db_theta_gri) < 1.0 ){
      cuts_gamma = 0;
    }
    // Zenith angle cut. Zenith angles for p and nuclei are taken from pass2 reconstruction and corrected with p mean bias.
    if (rusdgeom_.theta[2] + 0.5 > 55.0)
      cuts_anisotropy = 0;
    if (rusdgeom_.theta[2] + 0.5 > 45.0){
      cuts_spectrum = 0;
      cuts_composition = 0;
    }
    if (db_theta_gri > 55.0){
      cuts_gamma = 0;
    }

    double db_xcore = 1.2*( rufldf_.xcore[0] + RUSDGEOM_ORIGIN_X_CLF);
    double db_ycore = 1.2*( rufldf_.ycore[0] + RUSDGEOM_ORIGIN_Y_CLF);
    double db_dxcore = 1.2*(rufldf_.dxcore[0]);
    double db_dycore = 1.2*(rufldf_.dycore[0]);
    double db_S800_ldf = rufldf_.s800[0];
    double db_dS800_ldf = rufldf_.dsc[0]/rufldf_.sc[0];
    double db_E_ldf = energy;//rufldf_.energy[0];
    double db_mindist_p2 = 1.2*mindist(rusdraw_.site, rufldf_.xcore[0], rufldf_.ycore[0]);
// basic parameters of Dima reconstruction
    printf("ML %i %.6f", db_date, db_time);
    printf(" %lg %lg %lg %lg", rusdgeom_.theta[2] + 0.5, phi_ivanov(rusdgeom_.phi[2]), db_xcore, db_ycore);
    printf(" %i %lg %lg %lg", dcount_nz, db_S800_ldf, db_E_ldf, db_mindist_p2);
    printf(" %lg %lg %lg %lg %lg", rusdgeom_.dtheta[2], rusdgeom_.dphi[2], db_dxcore, db_dycore, db_dS800_ldf);
    printf(" %lg %lg %i %i", rusdgeom_.chi2[2], rufldf_.chi2[0], rusdgeom_.ndof[2], rufldf_.ndof[0]);
    printf(" %i %i %i %i", cuts_anisotropy, cuts_spectrum, cuts_composition, cuts_gamma);
//    printf(" %i", cuts_anisotropy);
    printf("\n");
// basic parameters of Grisha reconstruction (for photon analysis)
    printf("MLG");
    printf(" %lg %lg %lg %lg", db_theta_gri, db_phi_gri, db_xcore_gri, db_ycore_gri);
    printf(" %lg %lg %lg", db_S800_gri, E_gamma(db_S800_gri, db_theta_gri), db_mindist_gri);
    printf(" %lg %lg %lg", P, chi2_gri, ndof_gri); // P=chi^2_gri/ndof_gri
    printf("\n");
// additional parameters used in BDT analysis (see SD composition paper)
    printf("MLX %lg", (aprime*sqrt(S_X))/sqrt(db_S800_gri)); // Linsley front curvature
    printf(" %lg", AOP1200); 	 							 // Area-over-peak (AOP) 1200
    printf(" %lg", AOPc1);    								 // AOP slope
    printf(" %i", AOPcnt);    								 // number of detectors for AOP fit (not used in BDT analysis)
															 // number of non-zero detectors is omitted here as it is already output in "ML" string above
    printf(" %i", dcount_ng);								 // number of detectors excluded from the fit of the shower front by the reconstruction procedure
															 // chi^2/N_dof of joint LDF and geomery fit is omitted here as it is already output in "ML" string above
	for(int c1=0;c1<Sbcount;c1++) { 						 // five S_b parameters
		printf(" %lg",Sb[c1]/db_S800_gri);
		}
    printf(" %lg", 0.5*gl_ul_sum);			 // sum of all signals in all detectors
    printf(" %lg", (gl_ul_sum>0?gl_ul_diff/gl_ul_sum:0.0));  // asymmetry  of  the  summed signal  at  the  upper  and  lower layers of detectors
    printf(" %i", gl_pcnt);  								 // total number of peaks
    printf(" %i", gl_pcnt_largest); 						 // number of peaks for the detector with the largest signal
    printf(" %i", gl_apcnt_u); 								 // total number of peaks present in the upper layer and not in the lower
    printf(" %i", gl_apcnt_l); 								 // total number of peaks present in the lower layer and not in the upper.
    printf("\n");
//debug: dump of cuts values
//        printf("CUTS");
//        printf(" %i %i %lg %i %lg", rufptn_.nstclust, rusdgeom_.ndof[2], gfchi2pdof, rufldf_.ndof[0], ldfchi2pdof);
//        printf(" %lg %lg %lg %i", pdErr(rusdgeom_.theta[2], rusdgeom_.dtheta[2], rusdgeom_.dphi[2]), rufldf_.dsc[0], rufldf_.sc[0], event_surroundedness);
//        printf("\n");

    if(rusdraw_.event_code == 0) { // MC event
      double db_mindist_mc=1.2*mindist(rusdraw_.site, rusdmc_.corexyz[0]/1.0e5/1.2 - RUSDGEOM_ORIGIN_X_CLF,
			      rusdmc_.corexyz[1]/1.0e5/1.2 - RUSDGEOM_ORIGIN_Y_CLF);
      printf("MCEV %i %i %i", rusdmc_.event_num, rusdmc_.parttype, rusdmc_.corecounter);

      if(rusdmc_.parttype == 1) {printf(" %lg", rusdmc_.energy);}//for photon primaries MC energy is not rescaled
      else {printf(" %lg",rusdmc_.energy * enscaleFD);}//for other primaries MC energy is rescaled to proton FD energy

	   printf(" %lg %lg %lg %lg %lg %lg\n",
	   rusdmc_.theta*180.0/M_PI,
	   (tasdcalibev_.sim.primaryEnergy>1e-9)?tasdcalibev_.sim.primaryAzimuth:phi_ivanov(rusdmc_.phi*180.0/M_PI),
	   rusdmc_.height/1.0e5, rusdmc_.corexyz[0]/1.0e5,  rusdmc_.corexyz[1]/1.0e5,
	     db_mindist_mc);
    }
	...
	t_s = sqrt(dvalue[c1][6]);
      ...
      dti[c1].saturated = dsaturated[c1];
      dti[c1].badgeom = badgeom;
      dti[c1].x = x;
      dti[c1].y = y;
      dti[c1].z = z;
      dti[c1].S = S;
      dti[c1].t = t;
      dti[c1].t_curved = t-t0-r_plane;
   }
    ...
    for (dtit=dtvector.begin(); dtit!=dtvector.begin()+dcount; ++dtit) {
      ...
      (void)(ymax);
      for(int h=0; h<rufptn_.nhits; h++) {
	if( rufptn_.xxyy[h] != dtit->xxyy) continue;
       	if( rufptn_.isgood[h]<3 ) continue;

	for(int layer=0; layer<=1; layer++) {
	  for(int wf = rufptn_.wfindex[h];wf<rufptn_.wfindex[h]+rufptn_.nfold[h]; wf++) {
	    double tX = (double(rusdraw_.clkcnt[wf])/double(rusdraw_.mclkcnt[wf])-(rusdgeom_.tearliest - int(rusdgeom_.tearliest)))-t0/RUFPTN_TIMDIST*1e-6;
	    // time of the beginning of the waveform, seconds
	    printf("DT %04i %i %i %g %g %g %g", dtit->xxyy, dtit->saturated, dtit->badgeom, dtit->x, dtit->y, dtit->z, dtit->S);
	    printf(" %.9f %.9f %i WF", dtit->r_plane/RUFPTN_TIMDIST*1e-6, tX-dtit->r_plane/RUFPTN_TIMDIST*1e-6, layer);
	    for(int c2=0;c2<rusdraw_nchan_sd;c2++) {
	      double w;
	      w = 1.22*(double(rusdraw_.fadc[wf][layer][c2])- double(rusdraw_.pchped[wf][layer])/8.0)/rusdraw_.mip[wf][layer];
	      printf(" %g", w);
	    }
	    printf("\n");
	  }
...
double E_gamma(double S800, double theta) {
    double SMIP = S800/1.22;
    double x = 1.0/cos(theta/180.0*3.1415926) - 1.0/cos(28.65*3.1415926/180.0);
    double s00 = SMIP * exp(0.103*x + 2.400*x*x - 0.025*x/(x+0.15));
    double s01 = SMIP * exp(1.27 * x + 1.256 * x * x - 0.025 * x / (x + 0.15));
  if(theta < 45.) {
    return 0.336 * exp(log(s00) * 1.290);
  }
  else {
    return 0.264 * exp(log(s01) * 1.264);
  }
}
...
void plot_dtf(double *init, double P) {
  FILE *fil, *gp1, *gp2, *gp3, *gp4, *gp5;
    char fname_base[sd_fname_size], fname[sd_fname_size+0x10], cmd[sd_fname_size+0x1000];
    char gp_cmd1[gp_maxcmd+0x1000], 
      gp_cmd2[gp_maxcmd+0x1000], 
      gp_cmd3[gp_maxcmd+0x1000], 
      gp_cmd4[gp_maxcmd+0x1000], 
      gp_cmd5[gp_maxcmd+0x1000], 
      tmp_cmd[gp_maxcmd+0x1000], 
      title_cmd[gp_maxcmd];
    ...
    if(cmd_opt.isset("gnu")) {
      sprintf(title_cmd, "set title \"%li %.6f (%.2f, %.2f) ndet=%i angles: %.1f, %.1f S800=%.3g a=%.3g eta=%.3g%s chi2/N=%.4g",
	      ...
	      init[2]*180.0/M_PI, phi_ivanov(init[3]*180.0/M_PI), exp(init[5]), aprime, eta, fiteta?"":"fix", P);

      if(rusdmc_.event_num) {
	sprintf(tmp_cmd, "\\n MC (%.2f, %.2f) angles: %.1f, %.1f E=%.3g\";", rusdmc_.corexyz[0]/1.0e5,
		rusdmc_.corexyz[1]/1.0e5, rusdmc_.theta*180.0/M_PI, phi_ivanov(rusdmc_.phi*180.0/M_PI),
		rusdmc_.energy);
      }
      else {
	sprintf(tmp_cmd, "\";");
      ...
      fil = fopen(fname, "wb"); fprintf(fil, "%s", gp_cmd1); fclose(fil); 
      sprintf(fname, "%s/%s.gnu2", OUTDIR, fname_base);
      fil = fopen(fname, "wb"); fprintf(fil, "%s", gp_cmd2); fclose(fil); 
      sprintf(fname, "%s/%s.gnu3", OUTDIR, fname_base);
      fil = fopen(fname, "wb"); fprintf(fil, "%s", gp_cmd3); fclose(fil); 
      sprintf(fname, "%s/%s.gnu4", OUTDIR, fname_base);
      fil = fopen(fname, "wb"); fprintf(fil, "%s", gp_cmd4); fclose(fil); 
      sprintf(fname, "%s/%s.gnu5", OUTDIR, fname_base);
      fil = fopen(fname, "wb"); fprintf(fil, "%s", gp_cmd5); fclose(fil); 
	...
	fprintf(gp1, "%s", gp_cmd1); 	      fprintf(gp2, "%s", gp_cmd2);         fprintf(gp3, "%s", gp_cmd3);  	   fprintf(gp4, "%s", gp_cmd4);	 fprintf(gp5, "%s", gp_cmd5);
...
bool fix_site() {
  ...
  if(tower != rusdraw_.site) {
    if(cmd_opt.isset("v")) {
      fprintf(stderr,"Tower is fixed from %i ", rusdraw_.site);
    }
    rusdraw_.site = tower;
    if(cmd_opt.isset("v")) {
      fprintf(stderr,"to %i, nwf = %i, dataset = %i\n", rusdraw_.site, rusdraw_.nofwf, n_dataset);
    }
    return true;
  }
  return false;
